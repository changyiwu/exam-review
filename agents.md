...
- **要再加 `startPairing()` 的呼叫點時務必小心**：目前四個呼叫點都不可能在請求飛行中觸發（重試計時器已清、按鈕已被狀態文字取代、輪詢已停），所以只有一條鏈。一旦新增一個能在飛行中觸發的入口（例如某個按鈕、某個返回登入的路徑），`clearTimeout` 擋不住它——取消得了已排程的重試，取消不了飛行中的請求，而**過期的成功回應會把 `pairing` 覆蓋成舊通道，QR 就對不上真正在輪詢的那條**。程式裡已備妥 `pairingEpoch` 世代編號擋這種情況（`class-score` 有兩個外部呼叫點，實測會疊成兩條鏈，就是靠它修掉的）
